...
def func_2(callback):
    if callback.data == 'btn_5':
        ...
        bot.send_message(callback.message.chat.id, 'У входа в храм вы встречаете стража\nОн говорит что вам нужно рассказать ему анекдот и если ему понравится, то он вас пропустит, иначе - убьет')
...
def story_1(message):
    if message.text.lower() == 'лес':
        ...
        bot.send_message(message.chat.id, 'вы попали в измерение духа и вам нужно помочь найти эликсир')
        bot.send_message(message.chat.id, 'Согласившись на помощь герой попадает в храм с эликсиром')
        bot.send_message(message.chat.id, 'У входа в храм вы встречаете стража\nОн говорит что вам нужно рассказать ему анекдот и если ему понравится, то он вас пропустит, иначе - убьет')
        bot.register_next_step_handler(message, joke)
    else:
...
def joke(message):
    ...
    if num == 1 or num == 2:
        ...
        btn_2 = telebot.types.InlineKeyboardButton('Сразится с ними', callback_data='btn_12')
        markup.add(btn_1, btn_2)
        bot.send_message(message.chat.id, 'Стражу понравилась шутка и он пропустил вас')
        bot.send_message(message.chat.id, 'В храме вы встречается бандитов\nЧто вы хотите сделать?', reply_markup=markup)
    else:
        bot.send_message(message.chat.id, 'Стражу не понравилась шутка и он убил вас')
        bot.send_message(message.chat.id, 'Если хотите начать заново - /1')
# **********************************************************************************************************************************
